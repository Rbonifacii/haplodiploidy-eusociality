...
here::i_am("scripts/regressions/phyregB/phyreg_str_mw.R")

# ==== Load Data ====
# Load cleaned dataset
insect_data <- read.csv(here("data", "datasetS1.csv")) %>%
  select(species_name, eusocial_mw, haplodiploidy_strict) %>%
  filter(!is.na(haplodiploidy_strict), !is.na(eusocial_mw))  # Remove rows with missing values
...
phylo_model <- phyreg(
  control = eusocial_mw ~ 1,                  # Null model (intercept only)
...
# ==== Save Model Output ====
saveRDS(phylo_model, file = here("output", "phyreg_str_mw.rds"))
