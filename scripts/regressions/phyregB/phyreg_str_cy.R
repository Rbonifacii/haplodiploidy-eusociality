...
here::i_am("scripts/regressions/phyregB/phyreg_str_cy.R")

# ==== Load Data ====
# Load cleaned dataset
insect_data <- read.csv(here("data", "datasetS1.csv")) %>%
  select(species_name, eusocial_cy, haplodiploidy_strict) %>%
  filter(!is.na(haplodiploidy_strict), !is.na(eusocial_cy))  # Remove rows with missing values
...
phylo_model <- phyreg(
  control = eusocial_cy ~ 1,                  # Null model (intercept only)
  test = ~ haplodiploidy_strict,           # Test predictor
  data = insect_data,                         # Trait data
  phydata = tree_vector$phy                   # Phylogeny (internal format)
)

# ==== Save Model Output ====
saveRDS(phylo_model, file = here("output", "phyreg_str_cy.rds"))
