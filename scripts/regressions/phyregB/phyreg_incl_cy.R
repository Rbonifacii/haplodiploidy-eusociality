...
here::i_am("scripts/regressions/phyregB/phyreg_incl_cy.R")

# ==== Load Data ====
# Load cleaned dataset
insect_data <- read.csv(here("data", "datasetS1.csv")) %>%
  select(species_name, eusocial_cy, haplodiploidy_inclusive) %>%
  filter(!is.na(haplodiploidy_inclusive), !is.na(eusocial_cy))  # Remove rows with missing values
...
# ==== Save Model Output ====
saveRDS(phylo_model, file = here("output", "phyreg_incl_cy.rds"))
