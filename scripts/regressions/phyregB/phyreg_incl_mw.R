...
insect_data <- read.csv(here("data", "datasetS1.csv")) %>%
  select(species_name, eusocial_mw, haplodiploidy_inclusive) %>%
  filter(!is.na(haplodiploidy_inclusive), !is.na(eusocial_mw))  # Remove rows with missing values
...
# ==== Save Model Output ====
saveRDS(phylo_model, file = here("output", "phyreg_incl_mw.rds"))
