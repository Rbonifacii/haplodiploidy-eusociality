...
here::i_am("scripts/regressions/phyregB/phyreg_str_bg.R")

# ==== Load Data ====
# Load cleaned dataset
insect_data <- read.csv(here("data", "datasetS1.csv")) %>%
  select(species_name, superorganism_bg, haplodiploidy_strict) %>%
  filter(!is.na(haplodiploidy_strict), !is.na(superorganism_bg))  # Remove rows with missing values
...
# ==== Save Model Output ====
saveRDS(phylo_model, file = here("output", "phyreg_str_bg.rds"))
