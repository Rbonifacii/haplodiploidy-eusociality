...
set.seed(2025)
here::i_am("scripts/HMM/HMIM_incl_mw.R")
...
trait_data <- read.csv(here("data", "datasetS1.csv"), stringsAsFactors = FALSE) %>%
  select(sp = species_name, X = haplodiploidy_inclusive, S = eusocial_mw) %>%
...
# Save table
readr::write_csv(aic_table, here("scripts", "analyseschesters17", "output", "HMIM_AIC_table_incl_mw.csv"))
...
saveRDS(
  ...
  file = here( "output", "HMIM_model_fits_incl_mw.rds")
)
