...
set.seed(2025)
here::i_am("scripts/HMM/HMIM_str_bg.R")
...
trait_data <- read.csv(here("data", "datasetS1.csv"), stringsAsFactors = FALSE) %>%
  select(sp = species_name, X = haplodiploidy_strict, S = superorganism_bg) %>%
...
# Save table
readr::write_csv(aic_table, here("scripts", "analyseschesters17", "output", "HMIM_AIC_table_str_bg.csv"))
...
saveRDS(
  ...
  file = here("scripts", "analyseschesters17", "output", "HMIM_model_fits_str_bg.rds")
)
