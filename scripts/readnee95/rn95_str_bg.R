...
# - This script uses the "strict" definition of Haplodiploidy, and Boomsma & Gawnes "Superorganism" definition 
# ==============================================================================
...
haplo_vec <- setNames(insect_data$haplodiploidy_strict, insect_data$species_name)[tree$tip.label]
...
write.csv(contrasts_df, here("output", "read_nee_contrasts_str.csv"), row.names = FALSE)
...
contrast_results <- contrasts_df %>%
  ...
  mutate(
    ...
    haplo_eusocial = sum(insect_data$species_name %in% unlist(haplo_tips) & insect_data$superorganism_bg == "1"),
    haplo_total = length(unlist(haplo_tips)),
    sister_eusocial = sum(insect_data$species_name %in% unlist(sister_tips) & insect_data$superorganism_bg == "1"),
...
write.csv(contrast_results, here("output", "read_nee_results_str_bg.csv"), row.names = FALSE)
...
write.csv(eusocial_results, here("output", "read_nee_robustness_str_bg.csv"), row.names = FALSE)
