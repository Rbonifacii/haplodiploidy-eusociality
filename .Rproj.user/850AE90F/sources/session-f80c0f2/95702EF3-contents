...
here::i_am("scripts/regressions/robust_regression/ROBRT_str_cy.R")

tree <- read.tree(here("tree", "chestersklado_pruned.tree"))

insect_data <- read.csv(here("data", "datasetS1.csv")) %>%
  select(species_name, haplodiploidy_strict, eusocial_cy) %>%
...
# ==== Save Model Output ====
saveRDS(robust_results, here("output", "robust_regression_str_cy.rds"))
