...
# - This script uses the "strict" definition of Haplodiploidy, and Crespi & Yanega's eusocoality definition 
# ==============================================================================
...
contrast_results <- contrasts_df %>%
  ...
  mutate(
    ...
    haplo_eusocial = sum(insect_data$species_name %in% unlist(haplo_tips) & insect_data$eusocial_cy == "1"),
    haplo_total = length(unlist(haplo_tips)),
    sister_eusocial = sum(insect_data$species_name %in% unlist(sister_tips) & insect_data$eusocial_cy == "1"),
...
write.csv(contrast_results, here("output", "read_nee_results_str_cy.csv"), row.names = FALSE)
...
write.csv(eusocial_results, here("output", "read_nee_robustness_str_cy.csv"), row.names = FALSE)
