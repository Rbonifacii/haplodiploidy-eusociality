...
insect_data <- read.csv(here("data", "datasetS1.csv")) %>%
  select(species_name, haplodiploidy_inclusive, eusocial_mw) %>%
  filter(!is.na(haplodiploidy_inclusive), !is.na(eusocial_mw))
...
insect_data <- insect_data %>%
  mutate(
    ...
    eusocial_mw = as.numeric(as.character(eusocial_mw))
  )
...
model1 <- phyloglm(
  eusocial_mw ~ haplodiploidy_inclusive,
  data = insect_data,
  phy = tree,
...
# ===============================
# Save model
saveRDS(model1, file = "output/logistic_incl_mw.rds")
